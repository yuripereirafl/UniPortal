    ...
    def process_and_save_tickets(self, db: Session, glpi_tickets, month_str, categories_dict: dict = None):
        ...
        for t in glpi_tickets:
            ...
            try:
                ...
                # Regras antigas para chamados de Infraestrutura fechados antes de agosto de 2026
                if is_infra and rule:
                    ticket_date = date_close or date_open
                    if ticket_date and ticket_date < datetime(2026, 8, 1):
                        # Mapeamento antigo
                        old_limits = {
                            "VAZAMENTO": 2880, # 2 dias
                            "OFTALMOLOGIA": 21600, # 15 dias
                            "OFTALMO": 21600,
                            "EQUIPAMENTOS MÉDICOS": 14400, # 10 dias
                            "EQUIPAMENTOS MEDICOS": 14400,
                            "PREDIAL": 4320, # 3 dias
                            "CÂMERAS": 4320, # 3 dias
                            "CAMERAS": 4320,
                            "PINTURA": 10080, # 7 dias
                            "LUMINÁRIA": 7200, # 5 dias
                            "LUMINARIA": 7200,
                            "LUMINÁRIA CONSULTÓRIO": 2880, # 2 dias
                            "LUMINARIA CONSULTORIO": 2880,
                            "AR-CONDICIONADO": 10080, # 7 dias
                            "AR CONDICIONADO": 10080,
                            "COMPRAS": 10080, # 7 dias
                            "ELETRICA": 7200, # 5 dias
                            "ELÉTRICA": 7200,
                            "OUTROS": 10080 # 7 dias
                        }
                        clean_rule_name = rule.tipo.replace("CHAMADOS INFRAESTRUTURA - ", "").strip().upper()
                        if clean_rule_name in old_limits:
                            limit = old_limits[clean_rule_name]
