<template>
  <div class="tabela-funcionarios">
    ...
    <div v-if="showForm" class="modal-overlay">
      ...
      <div v-if="showAcessoNegado" class="modal-overlay" style="z-index: 9999;">
        <div class="form-modal" style="max-width: 400px; text-align: center;">
          <h3 style="color: #d32f2f;">Acesso negado</h3>
          <p>Você não tem permissão para realizar esta ação.</p>
          <button class="btn-cadastrar" @click="fecharModalAcessoNegado">Fechar</button>
        </div>
      </div>
      <div class="form-modal">
  ...
  methods: {
    ...
    abrirEditar(func) {
      ...
      this.form = {
        ...
        equipe: func.equipe || '',
        data_rescisao: func.data_rescisao || ''
      };
    ...
    async salvarEdicaoFuncionario() {
      ...
      try {
        await axios.put(`${API_BASE_URL}/funcionarios/${this.funcionarioEditId}`, {
          ...
          cpf: this.form.cpf,
          tipo_contrato: this.form.tipo_contrato,
          data_afastamento: this.form.data_afastamento || null,
          data_retorno: this.form.data_retorno || null,
          grupos_email_ids: [...this.form.grupos_email_ids],
          setores_ids: [...this.form.setores_ids],
          sistemas_ids: [...this.form.sistemas_ids],
          grupos_pasta_ids: [...this.form.grupos_pasta_ids],
          data_admissao: this.form.data_admissao || null,
          data_inativado: dataInativadoFormatada || null,
          equipe: this.form.equipe,
          data_rescisao: this.form.data_rescisao || null
        }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        await this.carregarFuncionarios();
    ...
    fecharModalAcessoNegado() {
      this.showAcessoNegado = false;
    },
  },
  mounted() {
